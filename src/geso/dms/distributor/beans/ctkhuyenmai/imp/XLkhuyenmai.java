...
public class XLkhuyenmai 
{
	...
	public String key_sanpham_sudung = "";
	...
	private Hashtable<String,String> HashD = new Hashtable<String,String>();
	...
	public XLkhuyenmai(String userId, String date, String khachhang,String dhid)
	{
		db = new dbutils();
		this.dhId=dhid;
	...
	{   
		...
		//SCHEME PHAN BO CHO NPP NAY
		// Loai CT = 4 là CTKM ĐẶC BIỆT KHÔNG LẤY VÀO
		sql = "\n select distinct a.loaingansach,a.pk_seq, a.scheme, a.tungay, a.denngay, a.loaict, a.kho_fk, isnull(a.diengiai,'') as diengiai, " +
		"\n d.ngansach, trakhuyenmai.ChietKhau, a.PHANBOTHEODONHANG  " +
		"\n 	,case when a.phanbotheodonhang =0 then isnull(ss.tonggiatri,0)  else isnull(ss.soxuat,0) end as dasudung " + 
		...
		"\n			where dh.trangthai !=2 and dh.npp_fk =  d.npp_fk and dhkm.ctkmid = d.ctkm_fk and not exists ( select 1 from donhangtrave where trangthai = 3 and donhang_fk = dh.pk_seq) " +
		"\n			group by dhkm.donhangId " +
		"\n 	)ss " +
		"\n )ss	" +
		"\n where a.apdungcho in (0,2) " +
		"\n and isnull(a.LEVEL_PHANBO,0) = 0 " +
		"\n and a.loaict <> '3' and b.chon = '1' " +
		"\n and a.tungay <= '" + date + "' and '" + date + "' <= a.denngay " +				
		"\n and f.SANPHAM_FK in (select sanpham_fk from DONHANG_SANPHAM where donhang_fk = '" + this.dhId + "') " +
		"\n AND " +
		"\n ( " +
		...
		"\n and " +
		"\n ( " +
		"\n    a.nhomkhnpp_fk is null " +
		"\n    or a.nhomkhnpp_fk in ( " +
		"\n        select pk_seq from nhomkhachhangnpp " +
		"\n        where pk_seq in (select nhomkhnpp_fk from ctkm_khachhang where khachhang_fk ='" + this.khachhang + "') " +
		"\n    ) " +
		"\n ) " +
		"\n and " +
		"\n ( " +
		"\n    a.nhom_kh_loai_tru is null " +
		"\n    or not exists  ( select 1 from ctkm_khachhang where khachhang_fk ='" + this.khachhang + "' and nhomkhnpp_fk =  a.nhom_kh_loai_tru  ) " +
		"\n ) " +
		"\n and " +
		"\n ( " +
		"\n     (select count(*) from CTKHUYENMAI_KBH where a.pk_seq = ctkm_fk ) = 0 " +
		"\n     or exists " +
		"\n     ( " +
		"\n         select 1 from CTKHUYENMAI_KBH " +
		"\n         where a.pk_seq = ctkm_fk " +
		"\n         and KBH_FK = (select KBH_FK from KHACHHANG where pk_seq = '" + this.khachhang + "') " +
		"\n     ) " +
		"\n ) " +
		"\n and isnull(a.isduyet,'0') = '1' and isnull(a.loaict,'') <> 4 "; 

		//SCHEME PHÂN BỔ CHO KHÁCH HÀNG NÀY
		sql += "\n UNION  ";
		sql += "\n select distinct  a.loaingansach,a.pk_seq, a.scheme, a.tungay, a.denngay, a.loaict, a.kho_fk, isnull(a.diengiai, '') as diengiai, " +
		"\n d.ngansach, trakhuyenmai.ChietKhau, a.PHANBOTHEODONHANG, " +
		...
		"\n     where CTKMID = d.CTKM_FK " +
		"\n     and DONHANGID in ( select PK_SEQ from DONHANG where NPP_FK = d.NPP_FK and TRANGTHAI != 2 ) " +
		"\n     and DONHANGID not in ( select DONHANG_FK from DONHANGTRAVE where DONHANG_FK is not null and NPP_FK = d.NPP_FK and TRANGTHAI in  (1,3))) " +
		"\n ,0) " + 
		"\n else ISNULL(( " +
		"\n     select sum( SOLUONG) " + 
		"\n     from DONHANG_CTKM_TRAKM    " + 
		"\n     where SPMA IS NOT NULL AND CTKMID = d.CTKM_FK " +
		"\n     and DONHANGID in (select PK_SEQ from DONHANG where NPP_FK = d.NPP_FK and TRANGTHAI != 2) " +
		"\n     and DONHANGID not in (select DONHANG_FK from DONHANGTRAVE where DONHANG_FK is not null and NPP_FK = d.NPP_FK and TRANGTHAI in (1,3))) " +
		"\n ,0) " +
		"\n end as dasudung " + 
		"\n from ctkhuyenmai a " +
		"\n inner join ctkm_npp b on a.pk_seq = b.ctkm_fk and b.NPP_FK = '" + this.nppId + "' " +
		"\n inner join phanbokhuyenmai d on b.CTKM_FK = d.CTKM_FK and d.KHACHHANG_FK = '" + this.khachhang + "' " +
		"\n inner join CTKM_DKKM e on a.pk_seq = e.CTKHUYENMAI_FK " + 
		"\n inner join DIEUKIENKM_SANPHAM f on e.DKKHUYENMAI_FK = f.DIEUKIENKHUYENMAI_FK " +
		"\n left join " +
		"\n ( " +
		...
		"\n and " +
		"\n (    a.nhomkhnpp_fk is null " +
		"\n      or a.nhomkhnpp_fk in (select pk_seq from nhomkhachhangnpp where pk_seq in " +
		"\n          (select nhomkhnpp_fk from ctkm_khachhang  where khachhang_fk ='" + this.khachhang + "') " +
		"\n      ) " +
		"\n ) " +
		"\n and " +
		"\n ( " +
		...
		System.out.println("1.1.LOAI DON HANG: " + this.loaidonhang);
		System.out.println("1.2.Chuoi lay ctkm hien huu: " + sql);
		...
		if (ctkm != null)
		{
			try 
			{
				while(ctkm.next())
				{
					...
					int loaingansach = ctkm.getInt("loaingansach");
					boolean cokm = false;
					
					if(loaingansach == 0)
						cokm = true;
					else if (ngansach - dasudung > 0)
						cokm = true;

					System.out.println("chap_nhan_giam_so_suat_km =" + this.chap_nhan_giam_so_suat_km);
					
					if (cokm || !this.chap_nhan_giam_so_suat_km )
					{
						...
						if(ctkm.getInt("PHANBOTHEODONHANG") == 1)
							km.setSo_suat_toi_da( ngansach - dasudung );
						else
							km.setSo_suat_toi_da( 9999999 );
						...
						if (rsTraKm != null)
						{
							...
							while(rsTraKm.next())
							{
								...
								Trakhuyenmai trakm = new Trakhuyenmai(trakmId,this.khachhang, nppId, date, kenh_kh, khoId);
	...
	{		
		...
		//Neu Co sort phai xet la Ap KM CHiet Khau hay Binh thuong truoc (CK cua Vifon chay dac biet)
		/*String query = "delete donhang_ctkm_dkkm where  donhang_fk =  '"+getIdDonhang()+"'   " ;
		 System.out.println("6.Chen donhang_dkkm: " + query);
		...
		//bo 2 chiet khau trung nhau, chi lay chiet khau lon nhat ---->> Vifon
	...
	public Ctkhuyenmai getSoxuattheoScheme(Ctkhuyenmai ctkm, Hashtable<String, Integer> hashSanpham)
	{
		...
		int soXuatKM = 0;
		int soXuatKM_0_tinh_ngan_sach = 0;
		...
		for(int i = 0; i < dkkmList.size(); i++)
		{
			...
			sl_0_tinh_ngan_sach = sl;
			//System.out.println("113.So xuat KM khi dang xet scheme ( " + ctkm.getscheme() + " ) la: " + sl);

			if(sl > 0 && ctkm.getPhanbotheoDH().equals("1") )
			{
				if(pt.equals("||"))
				{
					if( soXuatKM + sl > ctkm.getSo_suat_toi_da()  )
						sl  = (int) ctkm.getSo_suat_toi_da() - soXuatKM;
				}
				else
				{
					if(  sl > ctkm.getSo_suat_toi_da()  )
						sl  = (int) ctkm.getSo_suat_toi_da() ;
				}
			}
			...
			if (sl > 0)
			{
				...
				this.dieuChinhKM((Dieukienkhuyenmai)ctkm.getDkkhuyenmai().get(i), hashSanpham, HashB, HashD, sl, ctkm);

				if (i > 0 && pt == "&&") //dieu kien and cac sanpham
				{
					//cap nhat lai so xuat km
					soXuatKM = min(soXuatKM, sl);
					soXuatKM_0_tinh_ngan_sach = min(soXuatKM_0_tinh_ngan_sach, sl_0_tinh_ngan_sach);
					for(int j = i; j >= 0; j--)
					{							
						//dieu chinh lai slg sanpham trong hashSanpham (phai thay doi theo soxuat)
						this.dieuChinhKM2((Dieukienkhuyenmai)ctkm.getDkkhuyenmai().get(j), hashSanpham,HashD, soXuatKM);
					}
				}
				else //dieu kien or
				{
					soXuatKM += sl;
					soXuatKM_0_tinh_ngan_sach += sl_0_tinh_ngan_sach;
				}



			}
			else// set để ra thông báo
			{
				if (i > 0 && pt == "&&") //dieu kien and cac sanpham
				{
		...
		ctkhuyenmai.setSo_suat_toi_da(ctkm.getSo_suat_toi_da());
		...
		{		
			System.out.println("soXuatKM_0_tinh_ngan_sach = "+ soXuatKM_0_tinh_ngan_sach);
			System.out.println("soXuatKM = "+ soXuatKM);
			System.out.println("chap_nhan_giam_so_suat_km = "+ chap_nhan_giam_so_suat_km);
			
			if(!this.chap_nhan_giam_so_suat_km &&  soXuatKM_0_tinh_ngan_sach > soXuatKM)
				this.ctkm_bi_giam_so_suat.add( ctkhuyenmai.getscheme() + " số suất thực tế đạt "+soXuatKM_0_tinh_ngan_sach+" , số suất còn lại  " + soXuatKM );
		...
		else
		{
			...
			if(ctkm.getPhanbotheoDH().equals("1") && soXuatKM > ctkm.getSo_suat_toi_da())// phan bo theo so suat
				soXuatKM = (int) ctkm.getSo_suat_toi_da();
			
			System.out.println("soXuatKM = "+ soXuatKM_0_tinh_ngan_sach);
			System.out.println("soXuatKM = "+ soXuatKM_0_tinh_ngan_sach);
			System.out.println("chap_nhan_giam_so_suat_km = "+ chap_nhan_giam_so_suat_km);
			if(!this.chap_nhan_giam_so_suat_km &&  soXuatKM_0_tinh_ngan_sach > soXuatKM)
				this.ctkm_bi_giam_so_suat.add( ctkhuyenmai.getscheme() + " số suất thực tế đạt "+soXuatKM_0_tinh_ngan_sach+" , số suất còn lại  " + soXuatKM );
			...
			while(keyList.hasMoreElements())
			{
				...
				if (dkkm.getType() == 2)
				{
					...
					if (sanpham_soluong.get(key) != null)
					{
						...
						System.out.println("[Key]"+key+"[]"+sanpham_dongia.get(key));
					}
				}
				else
				{
	...
	private void dieuChinhKM(Dieukienkhuyenmai dkkm, Hashtable<String, Integer> sp_sl, Hashtable<String, Float> sp_dongia, Hashtable<String, String> sp_donhang, int soXuatKM, ICtkhuyenmai ctkm)
	{
		...
		/*for(int i = 0; i < sanphamList.size(); i++)
		{
			...
			if (i==0)
			{
				String query = "delete donhang_ctkm_dkkm where  donhang_fk =  '"+getIdDonhang()+"'  and dkkm_fk = '"+dkkm.getId()+"' " ;
				System.out.println("6.Chen donhang_dkkm: " + query);
			...
			{
				String query  = "Insert into donhang_ctkm_dkkm(ctkm_fk, dkkm_fk, soxuat, sanpham_fk, soluongmua, donhang_fk) " + 
				" select '" + ctkm.getId() + "','" + dkkm.getId() + "','" + ctkm.getSoxuatKM() + "', pk_seq, '" + sp.getSoluongcan() + "', '"+getIdDonhang()+"' from sanpham where ma = '" +sp.getMasp()  + "' ";
				System.out.println("6.Chen donhang_dkkm: " + query);
	...
	private void dieuChinhKM2(Dieukienkhuyenmai dkkm,  Hashtable<String, Integer> hashSanpham, Hashtable<String, String> sp_donhang, int soXuat)
	{
	...
	public Sanpham[] getSanpham_slg(Dieukienkhuyenmai dkkm, Hashtable<String, Integer> sanpham_sl, int soxuatKm) //truong hop tongluong > 0
	{
		...
		for(int i = 0; i < sanpham.length; i++)
		{
			...
			System.out.println("114. Min: (" + sanpham[i].getSoluongThungAvaiable() + "," + (tongluongKM - sum) + "): " + min(sanpham[i].getSoluongThungAvaiable(), tongluongKM - sum));
	...
	public Hashtable<String,String> getHashD()
	{
		return this.HashD;
	}

	public void setHashD(Hashtable<String,String> hash)
	{
		this.HashD = hash;
	}
	...
	public boolean checkConfirm() 
	{
		...
		// check đụng trường hợp bất kì trong trả sp,  nhìu điều kiện trả hoặc
		for(int x = 0 ; x <  this.ctkmResual.size(); x ++ )
		{
			ICtkhuyenmai ctkm = this.ctkmResual.get(x);
			...
			for(int i = 0; i < trakmList.size(); i++)
			{
				...
				if (trakm.getPheptoan()== 2 &&  trakmList.size() > 1 )
				{
					this.ctkmResual.get(x).setConfirm(true);
					flag = true;
				}
	...
	{/*
		...
		XLkhuyenmai xlkm = new XLkhuyenmai("100374","2013-01-08","468033");
		xlkm.setHashA(hashA);
		xlkm.setHashB(hashB);
	...
	public void setKey_sanpham_sudung() {
		...
		{
			ICtkhuyenmai ctkm = this.ctkmResual.get(i);
			for(int x = 0; x < ctkm.getDkkhuyenmai().size(); x++)
			{
				IDieukienkhuyenmai dk = ctkm.getDkkhuyenmai().get(x);
				List<ISanpham> sanphamList = dk.getSanphamList();
				for( int y = 0 ; y < sanphamList.size();y++ )
				{
					ISanpham sp = sanphamList.get(y);
					/*String query = "Insert into DONHANG_CTKM_DKKM_TEMP(ctkm_fk, dkkm_fk, soxuat, sanpham_fk, soluongmua, donhang_fk) " + 
			 		" values('" + ctkm.getId() + "','" + dkkm.getId() + "','" + ctkm.getSoxuatKM() + "',(select pk_seq from sanpham where ma = '" +sp.getMasp()  + "'), " + sp.getSoluongcan() + ", '"+getIdDonhang()+"')";
					*/
					String sql = " select pk_seq from sanpham where ma ='" +sp.getMasp()  + "' ";
					ResultSet rs = db.get(sql);
					String spId = "0";
					try {
						rs.next();
					...
					jsonObject.add("ctkm_fk", new JsonPrimitive(ctkm.getId()));
					jsonObject.add("dkkm_fk", new JsonPrimitive(dk.getId()));
					jsonObject.add("sanpham_fk", new JsonPrimitive(spId));
					jsonObject.add("soxuat", new JsonPrimitive(ctkm.getSoxuatKM()));
					jsonObject.add("soluongmua", new JsonPrimitive(sp.getSoluongcan()));
					NEW.add(jsonObject);
		...
		String jsString = NEW.toString();

		//System.out.println("mahoa :" + mahoa);
		//String giaima =  Utility.giaiMa(mahoa);
		//System.out.println("giaima :" + giaima);
		
		//String he = "";
	//.	String giaima2 =  Utility.giaiMa(he);
		//System.out.println("giaima2 :" + giaima2);
		this.key_sanpham_sudung = jsString;
		
	}
	public String getKey_sanpham_sudung() {
		return key_sanpham_sudung;
	}
