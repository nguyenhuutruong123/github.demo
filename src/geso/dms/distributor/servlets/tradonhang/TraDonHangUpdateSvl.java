...
@WebServlet("/TraDonHangUpdateSvl")
public class TraDonHangUpdateSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public TraDonHangUpdateSvl()
	{
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
	    ...
	    dtdhBean = new TraDonHang(id);
        dtdhBean.setUserId(userId); //phai co UserId truoc khi Init
        dtdhBean.init();
     
        session.setAttribute("dtdhBean", dtdhBean);
        String nextJSP = "/AHF/pages/Distributor/TraDonHangUpdate.jsp";
        
        if(request.getQueryString().indexOf("display") >= 0 ) 
        	nextJSP = "/AHF/pages/Distributor/TraDonHangDisplay.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(action.equals("save"))
		{
			if(id == "")
			{
				...
				{
					ITraDonHangList obj = new TraDonHangList();
		...
		else
		{
			if(action.equals("checkDtdh"))
			{
				...
				String sql = "select pk_seq, ngaynhap, npp_fk from donhang where pk_seq = '" + dhId + "' and trangthai = '1' AND NPP_FK='"+nppId+"' ";
