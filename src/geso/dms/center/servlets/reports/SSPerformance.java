...
public class SSPerformance extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nextJSP = "/AHF/pages/Center/SSPerformance.jsp";
	...
	private String setQuery( IStockintransit obj) {
		...
		query=
				"\n select gsTEN, gsMa, ctDs as ctDs, dsBan.DoanhSo as tdDS, ctSI.CHITIEU as ctSI, isnull(tdSI.td, 0) as tdSI,"+
				"\n SoDonHang as sodonhang, isnull(sodh, 0) as sodh, giatritbdonhang as giatritbdonhang, isnull(Gttbdh, 0) as gttbdh, "+
				"\n SoKhachHangMoi as sokhachhangmoi, isnull(soKHmoi.sokhmoi,0) as khmoi, "+
				"\n SoKhachHangMuaHang as sokhachhangmuahang, ISNULL(sokh,0) as sokhds, "+chuoiselct+", "+chuoiselctsi+"  "+ 
				"\n from  "+ 
				"\n	(  "+ 
				"\n		select gs.SmartId as gsMa,gs.ten as gsTen, sum(b.ChiTieu) as ctDs, sum(b.SoDonHang) as sodonhang, "+
				"\n 			sum(b.SoKhachHangMoi) as sokhachhangmoi, sum(b.SoKhachHangMuaHang) as sokhachhangmuahang, "+ 
				"\n				sum(b.GiaTriTBDonHang) as giatritbdonhang, b.GSBH_FK, a.Nam, a.Thang, "+chuoingoac+"  "+ 
				...
				"\n 	group by gs.SmartId, gs.ten, b.GSBH_FK, a.Nam, a.Thang, "+chuoingoac+" "+
				"\n )as ctBan  "+ 
				"\n left join   "+ 
				"\n (  "+ 
				"\n	 select a.GSBH_FK, SUM(b.SoLuong*b.dongiagoc) as DoanhSo, count(distinct b.DONHANG_FK) as sodh, "+
				...
				"\n	 group by a.GSBH_FK  "+ 
				"\n )as dsBan on dsBan.GSBH_FK=ctBan.GSBH_FK  "+ 
				"\n left join   "+ 
				"\n (  "+ 
				"\n    select t.GSBH_FK, "+chuoingoac+"  "+ 
				"\n    from  "+ 
				"\n	(  "+ 
				"\n	 select a.GSBH_FK, SUM(b.SoLuong) as SoLuong, c.NSP_FK  "+ 
				"\n	 from DONHANG a  "+ 
				"\n		inner join DONHANG_SANPHAM b on b.DONHANG_FK=a.PK_SEQ  "+ 
				"\n		inner join NHOMSANPHAM_SANPHAM  c on c.SP_FK=b.SANPHAM_FK  "+ 
				"\n	 where a.TRANGTHAI=1 and a.NgayNhap like  '"+fromDate+"%'   "+ 
				"\n	 group by a.GSBH_FK, c.NSP_FK  "+ 
				"\n	 )as t pivot  ( sum(SoLuong) for NSP_FK in ( "+chuoingoac+"  )) as t   "+ 
				"\n )as tdBan on tdBan.GSBH_FK=ctBan.GSBH_FK  "+
				"\n left join "+
				"\n ( "+
				"\n	select count(distinct a.pk_seq) as sokhmoi, d.GSBH_FK from KHACHHANG a inner join KHACHHANG_TUYENBH b on a.PK_SEQ = b.KHACHHANG_FK "+ 
				"\n	inner join TUYENBANHANG c on b.TBH_FK = c.PK_SEQ inner join DDKD_GSBH d on c.DDKD_FK = d.DDKD_FK"+
				"\n	where a.ngaytao like '"+fromDate+"%' and a.TRANGTHAI = 1 "+
				"\n	group by d.GSBH_FK  "+
				"\n )as soKHmoi on soKHmoi.GSBH_FK = ctBan.GSBH_FK "+			
				"\n left join   "+
				"\n (	"+
				"\n	select e.GSBH_FK, sum(aa.CHITIEU) as chitieu from CHITIEU a  "+
				...
				"\n	group by e.GSBH_FK"+
				"\n ) as ctSI on ctSI.GSBH_FK = ctBan.GSBH_FK "+
				"\n left join  "+
				"\n (	"+
				"\n	select e.GSBH_FK, SUM(b.soluongttduyet * b.DonGiaGoc) as td "+
				"\n	from ERP_DONDATHANG a inner join ERP_DONDATHANG_SANPHAM b on b.dondathang_fk=a.PK_SEQ "+
				"\n	inner join NHAPP_GIAMSATBH e on a.NPP_FK = e.NPP_FK  "+
				"\n	where 1=1  and a.NgayDonHang like '"+fromDate+"%' and a.TRANGTHAI in (2, 4) "+
				"\n	group by e.GSBH_FK "+
				"\n ) as tdSI on tdSI.GSBH_FK = ctBan.GSBH_FK "+
				"\n left join "+
				"\n ( "+
				"\n	select e.GSBH_FK, "+chuoingoacsumsi+" "+
				...
				"\n	group by e.gsbh_fk	"+
				"\n ) as ctSIn on ctSIn.GSBH_FK = ctBan.GSBH_FK "+
				"\n left join "+
				"\n ("+
				"\n	select t.GSBH_FK, "+chuoingoacsi+"  "+
				"\n    from  ( "+
				"\n		select e.GSBH_FK, c.NSP_FK, SUM(b.soluong * b.DonGiaGoc) as td  "+
				"\n		from ERP_DONDATHANG a "+
				"\n		inner join ERP_DONDATHANG_SANPHAM b on b.dondathang_fk=a.PK_SEQ   "+
				"\n		inner join NHAPP_GIAMSATBH e on a.NPP_FK = e.NPP_FK "+
				"\n		inner join NHOMSANPHAM_SANPHAM  c on c.SP_FK=b.SANPHAM_FK   "+
				"\n		where 1=1  and a.NgayDonHang like '"+fromDate+"%' and a.TRANGTHAI in (2, 4) "+
				"\n		group by e.GSBH_FK, c.NSP_FK "+
				"\n	)as t pivot  ( sum(td) for NSP_FK in ( "+chuoingoacsi+"  )) as t "+
				"\n ) as tdSIn on tdSIn.GSBH_FK = ctBan.GSBH_FK";
		
		System.out.println("[SSPerformance]"+query);
		return query;
	}
	...
	{   
		try{
			...
			File f= new File(getServletContext().getRealPath(getServletContext().getInitParameter("path"))+"\\SS_ThucHienChiTieuTT.xlsm");
		...
		{				
			...
			{ 				
				...
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 2 and tieuchi = 1 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 2 and tieuchi = 2 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 2 and tieuchi = 3 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 2 and tieuchi = 4 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 2 and tieuchi = 5 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
