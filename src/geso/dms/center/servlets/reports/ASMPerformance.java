...
public class ASMPerformance extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nextJSP = "/AHF/pages/Center/ASMPerformance.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    obj.setuserTen((String)session.getAttribute("userTen")==null? "":(String) session.getAttribute("userTen"));
		 ...
		 obj.setASMId(util.antiSQLInspection(geso.dms.center.util.Utility.antiSQLInspection(request.getParameter("asmId"))==null? "":util.antiSQLInspection(request.getParameter("asmId"))));
		...
		try{
			...
			if(action.equals("Taomoi")){
		        ...
		        response.setHeader("Content-Disposition", "attachment; filename=ASMThucHienChiTieuTT.xlsm");
	...
	private String setQuery( IStockintransit obj) {
		...
		query=
				"\n select asmTEN,asmMa, ctDs as ctDs, dsBan.DoanhSo as tdDS, ctSI.CHITIEU as ctSI, isnull(tdSI.td, 0) as tdSI,"+
				...
				"\n from  "+ 
				"\n	(  "+ 
				"\n		select v.PK_SEQ as vungId, asm.SmartId as asmMa, asm.ten as asmTen, sum(b.ChiTieu)  as ctDs, sum(b.SoDonHang) as sodonhang, "+
				...
				"\n		from ChiTieuNhanVien a inner join ChiTieuNhanVien_ASM  b on a.Pk_Seq=b.ChiTieuNhanVien_FK  "+ 
				"\n		left join  "+ 
				"\n		(    "+ 
				...
				"\n inner join ASM on asm.pk_seq = b.ASM_FK	"+
				...
				"\n group by v.PK_SEQ, asm.SmartId, asm.ten, a.Nam, a.Thang, "+chuoingoac+" "+
				"\n )as ctBan  "+ 
				"\n left join   "+ 
				"\n (  "+ 
				"\n	 select v.PK_SEQ as vungId, SUM(b.SoLuong*b.dongiagoc) as DoanhSo, count(distinct b.DONHANG_FK) as sodh, "+
				...
				"\n	 group by v.PK_SEQ  "+ 
				"\n )as dsBan on dsBan.vungId = ctBan.vungId  "+ 
				"\n left join   "+ 
				"\n (  "+ 
				"\n    select t.vungId, "+chuoingoac+"  "+ 
				"\n    from  "+ 
				"\n	(  "+ 
				"\n	 select v.PK_SEQ as vungId, SUM(b.SoLuong) as SoLuong, c.NSP_FK  "+ 
				"\n	 from DONHANG a  "+ 
				"\n		inner join DONHANG_SANPHAM b on b.DONHANG_FK=a.PK_SEQ  "+ 
				...
				"\n		inner join VUNG v on v.PK_SEQ = kv.VUNG_FK  "+
				"\n	 where a.TRANGTHAI=1 and a.NgayNhap like  '"+fromDate+"%'   "+ 
				"\n	 group by v.PK_SEQ, c.NSP_FK  "+ 
				"\n	 )as t pivot  ( sum(SoLuong) for NSP_FK in ( "+chuoingoac+"  )) as t   "+ 
				"\n )as tdBan on tdBan.vungId = ctBan.vungId "+
				"\n left join "+
				"\n ( "+
				"\n	select count(distinct a.pk_seq) as sokhmoi, v.PK_SEQ as vungId from KHACHHANG a inner join NHAPHANPHOI npp on npp.PK_SEQ = a.NPP_FK "+ 
				"\n	inner join KHUVUC kv on kv.PK_SEQ = npp.KHUVUC_FK "+
				"\n	inner join VUNG v on v.PK_SEQ = kv.VUNG_FK "+ 
				"\n	where a.ngaytao >= '"+fromDate+"%' and a.TRANGTHAI = 1 "+
				"\n	group by v.PK_SEQ "+
				"\n )as soKHmoi on soKHmoi.vungId = ctBan.vungId "+ 
				"\n left join   "+
				"\n (	"+
				"\n	select v.pk_seq as vungid, sum(aa.CHITIEU) as chitieu from CHITIEU a  "+
				...
				"\n	group by v.pk_seq"+
				"\n ) as ctSI on ctSI.vungid = ctBan.vungId "+
				"\n left join  "+
				"\n (	"+
				"\n	select v.pk_seq as vungId, SUM(b.soluongttduyet * b.DonGiaGoc) as td "+
				...
				"\n	group by v.pk_seq "+
				"\n ) as tdSI on tdSI.vungId = ctBan.vungId "+
				"\n left join "+
				"\n ( "+
				"\n	select v.pk_seq as vungId, "+chuoingoacsumsi+" "+
				...
				"\n group by v.pk_seq"+
				"\n ) as ctSIn on ctSIn.vungId = ctBan.vungId "+
				"\n left join "+
				"\n ("+
				"\n	select t.vungId, "+chuoingoacsi+"  "+
				"\n    from  ( "+
				"\n		select v.pk_seq as vungId, c.NSP_FK, SUM(b.soluong * b.DonGiaGoc) as td  "+
				...
				"\n ) as tdSIn on tdSIn.vungId = ctBan.vungId";

		System.out.println("[ASMperformance]"+query);
    ...
    {   
		try{
			...
			File f= new File(getServletContext().getRealPath(getServletContext().getInitParameter("path"))+"\\ASM_ThucHienChiTieuTT.xlsm");
	...
	private void CreateStaticHeader(Workbook workbook, IStockintransit obj) 
	{
		...
		cell.setValue("TÌNH HÌNH THỰC HIỆN CHỈ TIÊU ASM");
	...
	private void FillData(Workbook workbook,IStockintransit obj, int row, String query)throws Exception 
	{
		...
		{				
			...
			{ 				
				...
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				cell.setValue(rs.getString("asmMa"));
				ReportAPI.setCellBackground(cell, Color.WHITE, BorderLineType.THIN, false, 0);

				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				cell.setValue(rs.getString("asmTen"));
				ReportAPI.setCellBackground(cell, Color.WHITE, BorderLineType.THIN, false, 0);
				...
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 3 and tieuchi = 1 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 3 and tieuchi = 2 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 3 and tieuchi = 3 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 3 and tieuchi = 4 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
				...
				phantramMTD = phantramMTD * 100;
				sql = "select tieuchi, TU, den, thuong from TIEUCHITINHTHUONG a inner join TIEUCHITINHTHUONG_CHITIET b on a.PK_SEQ = b.TIEUCHITINHTHUONG_FK "+ 
						  "\n	where TRANGTHAI = 1 and LOAI = 3 and tieuchi = 5 and nam = "+fromYear+" and thang = '"+fromMonth + "' and "+phantramMTD+" >= tu and "+phantramMTD+" <= den";
				rsthuong = db.get(sql);
				next = rsthuong.next();
				cell = cells.getCell(GetExcelColumnName(indexCol++) + indexRow);
				if(next) cell.setValue(rsthuong.getDouble("Thuong"));
