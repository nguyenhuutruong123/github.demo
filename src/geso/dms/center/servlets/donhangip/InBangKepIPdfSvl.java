...
public class InBangKepIPdfSvl extends HttpServlet 
{
	private static final long serialVersionUID = 1L;
	public InBangKepIPdfSvl() {
		super();
	}
	...
	private void CreatePxk(Document document, ServletOutputStream outstream, List<ISanphamIP> spList, List<ISanphamIP> spkmlist,Hashtable<String, Float> scheme_tongtien,Hashtable<String, Float> scheme_chietkhau, IDonhangIP dhBean, dbutils db) throws IOException
	{
		try
		{		
			String tennpp = "";
			...
			String query=
					"	select d.ten as khoTen,c.donvi as spDonVi, b.ma as spMa,b.ten as spTen,a.SoLuong,0 GiaMua,0 DonGiaGoc, isnull(ptCKDH,0) as ckDH  "+
							"	,isnull(ptCKTT,0) as ckTT,isnull(ptCKDLN,0) as ckDLN,isnull(ptCKTT,0)+isnull(ptCKDH,0)+isnull(ptCKDLN,0) as TongCK,(select isnull(chietkhau,0) from donhangIP where pk_seq = a.donhang_fk )as chietkhau ,(select isnull(Sotiengiam,0) from donhangIP where pk_seq = a.donhang_fk )as sotiengiam "+
			...
			try
			{
				while(rs.next())
				{
					...
					totalle+= thanhTien;
					String[] arr = new String[]{Integer.toString(i+1), spMa, spTen, khoTen,formatter.format(SoLuong ), spDonVi ,formatter2.format(DonGiaGoc), formatter1.format( totalCK )  ,"", formatter1.format( thanhTien ) };
					for(int j=0; j < 10; j++)
					{
			...
			cell12 = new PdfPCell();
			cell12.setHorizontalAlignment(Element.ALIGN_CENTER);
			tongtien =new Paragraph(formatter.format(tongtiensauthue - tongtienkhuyenmai-totalTienCK-sotiengiam) , font2);
			tongtien.setAlignment(Element.ALIGN_RIGHT);
			cell12.addElement(tongtien);
			table.addCell(cell12);
	...
	private String XuatFileExcel(HttpServletResponse response, String id) throws IOException
	{
		...
		try 
		{
			...
			response.setHeader("Content-Disposition", "attachment; filename=DonHangIP_" +getDateTime()+".xls");
