...
public class StockInTransitNPP extends HttpServlet {
	...
	public String getQuery(HttpServletRequest request,String sql,String userId)
	{
     String   query = 	"\n SELECT KBH.PK_SEQ AS KENHID, KBH.TEN AS KENH,	VUNG.PK_SEQ AS VUNGID, VUNG.TEN AS VUNG,NHAPHANG.NGAYNHAN as Pridate," +
     					...
     					"\n SP.MA AS SPMA, SP.TEN AS SPTEN, NHAPHANG_SP.SOLUONG AS QUANTITY, NHAPHANG_SP.SOLUONG*bg_npp_sp.GIAMUANPP AS AMOUNT ,TT.TEN AS TINHTHANH, " +
     					"\n Nhaphang.SO_Number as SoSO, NHAPHANG.dondathang_fk AS SoPO, kho.ten as tenkho, '' as donvi, " +
     					...
     					"	else \n"+
     					"	 NHAPHANG_SP.soluong end as soluongthung "+
...
//		    System.out.println(userId);
			...
			String nextJSP = "/AHF/pages/Distributor/StockInTransitNPP.jsp";
	...
	private void CreatePivotTable(OutputStream out,HttpServletResponse response,HttpServletRequest request,String[] manghien,IStockintransit obj ,String query)throws Exception
	{
		try 
		{
			...
			String fstreamstr = getServletContext().getInitParameter("path") + "\\HangChuaNhapKhoNPP.xlsm";
			...
			//File f= new File(getServletContext().getRealPath(getServletContext().getInitParameter("path"))+"\\HangChuaNhapKhoNPP.xlsm");
	...
	private void CreateHeader(Workbook workbook,IStockintransit obj) 
	{
		...
		cell = cells.getCell("AM1");		cell.setValue("NgayChungTu");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AN1");		cell.setValue("SoLuongLe");			ReportAPI.setCellHeader(cell);	    
		cell = cells.getCell("AO1");		cell.setValue("SoTien");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AP1");		cell.setValue("SoPO");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AQ1");		cell.setValue("SoSO");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AR1");		cell.setValue("Kho");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AS1");		cell.setValue("Soluongthung");			ReportAPI.setCellHeader(cell);
	}
	private void FillData(Workbook workbook,String[] manghien,String query) throws Exception 
	{
	    ...
	    while (rs.next()) {
			...
			cell = cells.getCell("AI" + String.valueOf(index));		cell.setValue(rs.getString("SPMA"));
			cell = cells.getCell("AJ" + String.valueOf(index));		cell.setValue(rs.getString("SPTEN"));
			...
			cell = cells.getCell("AM" + String.valueOf(index));		cell.setValue(rs.getString("Pridate"));
			cell = cells.getCell("AN" + String.valueOf(index));		cell.setValue(Float.parseFloat(rs.getString("QUANTITY"))); //14
			cell = cells.getCell("AO" + String.valueOf(index));		cell.setValue(Float.parseFloat(rs.getString("AMOUNT")));   //15	
			cell = cells.getCell("AP" + String.valueOf(index));		cell.setValue(rs.getString("SOPO"));
			cell = cells.getCell("AQ" + String.valueOf(index));		cell.setValue(rs.getString("SOSO"));
			cell = cells.getCell("AR" + String.valueOf(index));		cell.setValue(rs.getString("tenkho"));
			cell = cells.getCell("AS" + String.valueOf(index));		cell.setValue(rs.getString("soluongthung"));
			index++;
		} 
	    if(rs!=null) rs.close();
	    if(db!=null)
	    ...
	    Hashtable<String, Integer> selected = new Hashtable<String, Integer>();
			    ...
			    selected.put("istributor_code",11);
