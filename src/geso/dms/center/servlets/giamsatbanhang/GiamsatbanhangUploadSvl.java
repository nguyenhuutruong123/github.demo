...
import geso.dms.center.beans.giamsatbanhang.IGiamsatbanhangList;
import geso.dms.center.beans.giamsatbanhang.imp.Giamsatbanhang;
import geso.dms.center.beans.giamsatbanhang.imp.GiamsatbanhangList;
...
public class GiamsatbanhangUploadSvl extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet
{
	static final long serialVersionUID = 1L;
	private PrintWriter out;

	public GiamsatbanhangUploadSvl()
	{
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		Giamsatbanhang ddkdBean = new Giamsatbanhang(id);
		ddkdBean.init();
		...
		session.setAttribute("userId", userId);
		String nextJSP = "";
		nextJSP = "/AHF/pages/Center/GiamsatbanhangUploadUpdate.jsp";			
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String userId = null;
		String action = "", view = "";	
		String folderupload = getServletContext().getInitParameter("Anh_GSBH");
		dbutils db = new dbutils();
		Giamsatbanhang ddkdBean = new Giamsatbanhang(db);
				...
				for (FileItem item : items) {
					if (item.isFormField()) {
						...
						if (x != null && x.length() > 0){
							...
							if (!matcher.matches()) {
								ddkdBean = new Giamsatbanhang(id);							
								ddkdBean.init();
								ddkdBean.setUserId(userId);
							...
							imgList.add("GSBH_"+id+temp_name+matcher.group(2));
							...
							File uploadedFile = new File(folderupload+"\\\\", "GSBH_"+id+temp_name+matcher.group(2));
							try {
