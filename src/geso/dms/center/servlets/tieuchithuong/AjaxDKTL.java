...
public class AjaxDKTL extends HttpServlet 
{
	...
	private String createDKKM(String diengiai, String loaidieukien, String hinhthuc, String sotong, String sanpham, String nguoitao, String isThung)
	{
		...
		try 
		{
			...
			String query = "Insert into Dieukienkhuyenmaitl(diengiai, tongluong, tongtien, loai, ngaytao, nguoitao, ngaysua, nguoisua, IsThung) values(";
			...
			if(!db.update(query))
			{
				...
				msg = "Khong the tao moi 'Dieukienkhuyenmaitl', " + query;
				return msg; 
			}
			...
			if(sanpham.length() > 0)
			{
				//lay dkkm current
				String dkkkmCurrent = "";
				query = "select IDENT_CURRENT('Dieukienkhuyenmaitl') as dkkmId";
				...
				{	
					...
					for(int i = 0; i < masp.length; i++)
					{
						if(masp[i].length() > 0)
						{
							...
							query = "Insert into dieukienkmtl_sanpham(dieukienkhuyenmai_fk, sanpham_fk, soluong, batbuoc) " +
				...
				else
				{
					...
					for(int i = 0; i < masp.length; i++)
					{
						...
						masanphamTest += pk_seq + ",";
						query = "Insert into dieukienkmtl_sanpham(dieukienkhuyenmai_fk, sanpham_fk, soluong, batbuoc) values('" + dkkkmCurrent + "', '" + pk_seq + "', null, null)";
