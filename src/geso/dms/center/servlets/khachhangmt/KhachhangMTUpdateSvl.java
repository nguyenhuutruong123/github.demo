			...
			*/
			...
			if(id == null){
				if(nppBean.checkma(ma)){
					nppBean.setError("* Mã khách hàng đã tồn tại!");
					nppBean.setMessage("Mã Khách hàng đã tồn tại!");
					error = true;
				}
			}
			if(ma==""){
				nppBean.setError("* Vui lòng nhập mã Khách hàng!");
				nppBean.setMessage("Vui lòng nhập mã Khách hàng!");
				error = true;
			}
