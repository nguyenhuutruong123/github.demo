...
public class  Xacnhanuser implements IXacnhanuser
{
	...
	{ 
		...
		try
		{
			while(rs.next())
			{
				this.msg+="Nhà phân phối đã có đơn hàng XX \n";
				return;
			}
			if(rs!=null)rs.close();
		} catch (SQLException e)
	...
	{	
		...
		try
		{
			...
			query=     "  SELECT SUM(SOLUONG) AS TONCUOI,NPP_FK,KBH_FK,KHO_FK,SANPHAM_FK  " +  
					"  FROM   (    " +  
					"		SELECT DH.NPP_FK,DH.KBH_FK,DHSP.KHO_FK,DHSP.SANPHAM_FK,  SUM(SOLUONG) AS SOLUONG ,  " +  
					"		N'4.Xuất hàng bán' AS TYPE ,NGAYNHAP  			  " +  
					"		FROM DONHANG_SANPHAM DHSP    			  " +  
					"		INNER JOIN DONHANG DH ON DH.PK_SEQ = DHSP.DONHANG_FK  	    " +  
	...
	public void GuiEmail_Xacnhan() {
		try {
			...
			String table_str = "  "
					+ "   Dear Quý khách hàng, <br>  "
					+ "   Đây là email tự động gửi số lượng bản quyền Quý khách hàng hiện đang sử dụng trên hệ thống SalesUp DMS <br>  "
					+ "   <br> "
					+ "   Người xác nhận : "+userten+" "
					+ "   <br> "
					+ "   Ngày giờ xác nhận: "+this.getDateTime1()+" <br>  "
					+ " <TABLE border=1  > " + "<tr> "
					+ " <th align='center'> Loại đối tượng </th>" + " <th align='center'> Số lượng hoạt động </th>"
					+ " <th align='center'> Số lượng hoạt động có doanh số </th>"
			...
			rs.close();
			table_str += "</TABLE> <br> ";
			table_str+=" <br> Danh sách chi tiết như sau :  <br>  "
					+ " <TABLE border=1  > " + "<tr> "
					+ " <th> Đối tượng </th>" + " <th> Mã</th>  <th> Tên</th> <th> Hoạt động</th> <th> Có doanh số</th>"
			...
			while (rs.next()) {
				table_str += "<tr> " + " <th align='left'> " + rs.getString("type")
						...
						+ " </th>" + " <th align='left'> " + (rs.getString("trangthai").equals("1")?"Có":"Không")
						+ " </th>" + " <th align='left'> " + (rs.getString("IS_CODS").equals("1")?"Có":"Không")
						+ " </th>" + "</tr>  ";
