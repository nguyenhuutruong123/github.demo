...
public class Phuongxa implements IPhuongxa
{
	...
	public boolean CreatePx() {
		...
		data= new Object[]   {this.diengiai, this.qhId , this.ttId , this.ngaytao , this.ngaytao,this.nguoitao,this.nguoitao,this.trangthai };
		...
		command = "update PhuongXa set diengiai = cast(TinhThanh_FK as varchar) + '-' + cast(QuanHuyen_FK as varchar) + '-' + dbo.ftBoDau(Ten) where pk_seq = ?";
		data= new Object[]   {id};
		if( this.db.updateQueryByPreparedStatement(command, data)!=1 ) 
		{
	...
	public boolean UpdatePx() {
		...
		command = "UPDATE PHUONGXA SET TEN = ?, QUANHUYEN_FK =?, TINHTHANH_FK = ?, " +
				  "TRANGTHAI = ?, NGAYSUA =?, NGUOISUA = ? WHERE PK_SEQ = ? ";
		data= new Object[]   {this.diengiai, this.qhId , this.ttId , this.trangthai , this.ngaysua,this.nguoisua,this.id };
		db.viewQuery(command, data);
		//if (!db.update(command))
		if( this.db.updateQueryByPreparedStatement(command, data)!=1 ) 
		{
