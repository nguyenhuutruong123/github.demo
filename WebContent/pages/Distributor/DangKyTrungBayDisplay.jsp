...
<form name="dktbForm" method="post" action="">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
                            ...
                            if(cttrungbay != null){
					  		try{ while(cttrungbay.next()){ 
		    					if(cttrungbay.getString("cttbId").equals(dktbBean.getCttbId())){   
		    						tenchuongtrinh= cttrungbay.getString("cttbTen");
		    						
		    						%>
		      					<option   value='<%=cttrungbay.getString("cttbId")%>' selected><%= cttrungbay.getString("scheme")%> </option>
		      				<%}else{ %>
		     					<option disabled="disabled" value='<%=cttrungbay.getString("cttbId")%>'><%=cttrungbay.getString("scheme") %></option>
				  		...
				  		%>
	    					 <tr class="tbdarkrow">
			                    ...
			                    <% if(khList.getString("dat").length() > 0) { if(khList.getInt("dat") <= 0 ) { %>
				                    ...
				                    <% } %>
			                    <% }  else { %>
			                    		<input type="text" name="dangky" id="<%= "dangky" + Integer.toString(m) %>" value="<%= khList.getString("dangky") %>" style="text-align:right" size="6" readonly="readonly" >
			                    <% } } else { %> 
			                    		<% if( khachhang_dangky.contains(khList.getString("khId") ) ) {  %>
			                    	<input type="text" name="dangky" id="<%= "dangky" + Integer.toString(m) %>" value="<%= khachhang_dangky.get(khList.getString("khId")) %>" style="text-align:right" size="6" 
			                    			onkeypress="return keypress(event);" onkeyup="Check('<%= "dangky" + Integer.toString(m) %>')" >
