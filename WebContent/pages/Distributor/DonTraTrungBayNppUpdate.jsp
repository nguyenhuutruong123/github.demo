...
<% 
	...
	ResultSet hopdong = (ResultSet)lsxBean.getHopdongRs();
               ...
               <TR>
                ...
                <TD class="plainlabel" >Hợp đồng</TD>
                    <TD class="plainlabel"  colspan="1" >
                    	<select name = "hopdong_fk" style="width: 200px" class="select2" onChange="submitform2()" >
                    	<option value = "">
                        	<%
	               			try
	               			{
	               				...
	               				while(hopdong.next())
	               				{  
	                			if( hopdong.getString("pk_seq").equals(lsxBean.getHopdong_fk())){ %>
	                				<option value="<%= hopdong.getString("pk_seq") %>" selected="selected" ><%= hopdong.getString("ten") %></option>
	                			<%}else { %>
	                				<option value="<%= hopdong.getString("pk_seq") %>" ><%= hopdong.getString("ten") %></option>
	                		 		<% 	} 
	                			}
                    ...
                    <TD class="plainlabel" >Số sản phẩm trong Hợp đồng</TD>
                    <TD class="plainlabel"  colspan="1" ><input type ="text" value='<%=lsxBean.getSoluongsptronghopdong()%>' style="text-align:right" readonly="readonly" id="sosptronghopdong" name = "sosptronghopdong" >
                    </TD>
                </TR>
                <% } %>
                ...
                <TR>
                    <TD class="plainlabel" >Hóa đơn giảm nợ </TD>
                    <TD class="plainlabel"  colspan="3" >
                    	<select name = "hdId" style="width: 200px" class="select2"  >
                        	<option value="" >Chọn hóa đơn </option>
                        	<%
                        		ResultSet hdRs = lsxBean.getHdRs();
                        		if(hdRs != null)
                        		{
                        			try
                        			{
                        			while(hdRs.next())
                        			{  
                        			if( hdRs.getString("pk_seq").equals(lsxBean.getHdId())){ %>
                        				<option value="<%= hdRs.getString("pk_seq") %>" selected="selected" ><%= hdRs.getString("ten") %></option>
                        			<%}else { %>
                        				<option value="<%= hdRs.getString("pk_seq") %>" ><%= hdRs.getString("ten") %></option>
                        		 		<% 	} 
                        			} hdRs.close();} catch(SQLException ex){}
                        		}
                        	%>
                    	</select>
                    </TD>
