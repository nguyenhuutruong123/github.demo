...
for(int xy = 0; xy < xlkm.getCtkm_bi_giam_so_suat().size(); xy++)
{
	msg_vuot_ngan_sach +="\n " +  xlkm.getCtkm_bi_giam_so_suat().get(xy);
}
...
<head>
	...
	function thongbao_vuot_ngan_sach() {
		var tt = document.forms['kmForm'].msg_vuot_ngan_sach.value;
		if (tt.length > 0)
		{
			if(!confirm('Các CTKM bị hết ngân sách bạn có muốn tiếp tục lưu khuyến mãi?' + tt )) 
			{
				
				 document.forms['kmForm'].action.value='backForm';
		  		 document.forms['kmForm'].submit();
		  		   
			}else
			{
				 
				 document.forms['kmForm'].chap_nhan_giam_so_suat_km.value='true';
		  		 submitform();
			}
		}

		document.forms['kmForm'].msg_vuot_ngan_sach.value = '';
	}
		  			...
		  			for(j = 0; j < trakmId.length; j++)
		  			{
		  				if(trakmType.item(j).value == "3" && trakmHinhthuc.item(j).value == "2")
		  				{
		  					if(trakmPrimary.item(j).value == 0 && parseInt(tongsoxuat.value) >= 1 )  //tra sanpham (0),   tra tien (1)
		  					{
				 				...
				 				if( checkSlgTraKm_save(schemeList.item(pos).value + "." + trakmId.item(j).value + '.soluong') == false)
				 		  		{
				 		  			alert('Vui lòng điều chỉnh lại số lượng trả khuyến mãi bằng tổng lượng được hưởng');
				 	 				 return;
				 		  		}
  		...
  		function checkSpIds() //kiem tra ma san pham tra km da duoc chon so luong hay chua
		{
  		...
  		function Auto(checkId)
  		{
  			...
  			var k = document.getElementById(idTlg + ".tra_tongluong").value;  			
  		...
  		function checkSlgTraKm_save(name)
  		{
  			var sum = 0;
...
<INPUT type="hidden" name="sudungkhuyenmai" value='<%= Utility.dongMa_ver1_noreplace(xlkm.getKey_sanpham_sudung())  %>'>
<INPUT type="hidden" name="dhId" value='<%=xlkm.getIdDonhang() %>'>
<INPUT type="hidden" name="chap_nhan_giam_so_suat_km" value='<%= xlkm.getChap_nhan_giam_so_suat_km() %>'>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr height="25px">
		...
		<TD colspan="2" align="right" class="tbnavigation">Chào mừng bạn    <%= xlkm.getNppTen() %> &nbsp;</TD>
	</tr>
</table>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<TR class="tbdarkrow">
 ...
 <fieldset  >
 	...
 	<tr>
	    ...
	    <%
	    	...
	    	while(m < listCTKM.size())
	    	{
	    		...
	    		System.out.println("dungsanpham = "+ listCTKM.get(m).getDungsanpham() );
	    		if(ctkm.getDungsanpham().equals("1"))
	    		{
	    			%>
					<li style="color: red" onMouseOver="this.style.color='black'" onMouseOut="this.style.color='red'"><%= ctkm.getscheme() + " - " + diengiai %> 
    	...
    	<td >
	     ...
	     <ul  style="cursor:pointer; width:60% ">    
	    <%
	    	m = 0;
	    	while(m < listCTKM.size())
	    	{
	    		Ctkhuyenmai ctkm1 = (Ctkhuyenmai)listCTKM.get(m);
	    		String diengiai1 = ctkm1.getDiengiai(); 
	
	    		if(!ctkm1.getDungsanpham().equals("1"))
	    		{
	    		%>
	    		
	    			<li style="color: white" ><%= ctkm1.getscheme() + " - " + diengiai1 %> 
						<input type="hidden" name="Scheme" value="<%= ctkm1.getscheme() %>"></li>
	    <%		
	    		}
    ...
    <% if(xlkm.getDieuchinh() == false && xlkm.getDungDieuKien() == true){ %>
    	<tr><td colspan="2" style="color:white;"><input type="checkbox" id="ShowAll" name="ShowAll" checked="checked" value='1' onchange="submitform()"><i> Hiện tất cả các suất khuyến mãi ( tối đa ) có thể đạt được của đơn hàng </i></td></tr>
    <%}else{ %>
    	<tr><td colspan="2" style="color:white;"><input type="checkbox" id="ShowAll" name="ShowAll"  value='0' onchange="submitform()"><i> Hiện tất cả các suất khuyến mãi ( tối đa ) có thể đạt được của đơn hàng </i></td></tr>
    <%} %>
    </table>
</fieldset>
...
<fieldset>
           							...
           							<% } else {
	       								...
	       								%>	           								
		           							...
		           							<%}%>
	           									...
	           									<input type="hidden" id="<%= ctkm.getId() + "." + trakm2.getId() + ".tra_tongluong" %>" value="<%= Math.round((trakm2.getTongluong() * ctkm.getSoxuatKM()) - 0.5) %>">
    ...
    %>
    <%
   for( int xu = 0; xu < xlkm.getTichluyIdList().size(); xu++)
	{%>
    	<input type="hidden" name="chonTichluyDacBiet" value="<%= xlkm.getTichluyIdList().get(xu) %>" >
    <%}%>
