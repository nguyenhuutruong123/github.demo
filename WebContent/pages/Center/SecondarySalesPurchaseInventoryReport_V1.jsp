...
<% 
   ...
   int[] quyen = new  int[6];
   quyen = util.Getquyen("SecondarySalesPIRTT","", userId); 
...
String url = util.getChuyenNguUrl("SecondarySalesPIRTT_V1","",nnId);	
	%>
...
function seach()
{
	...
	function submitchitiet() {
		...
		document.getElementById("btnSave1").innerHTML = "<a href='#'><img src='../images/waiting.gif'  title='cho thuc hien..' border='1' longdesc='cho thuc hien..' style='border-style:outset'> Processing...</a>";		
		document.forms['frm'].action.value= 'taochitiet';
		document.forms["frm"].submit();
		reset();
	//	for ( var i = 0; i < fieldShow.length; ++i) {
	//		fieldShow.item(i).checked = false;
...
<!-- END RENDER AUTO -->
	...
	<form name="frm" method="post" action="../../SecondarySalesPIRTT_V1">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
	...
	<input type="hidden" id="checktontai" name="checktontai" value='<%=obj.getKey()%>'>
		<div id="main" style="width: 99%; padding-left: 2px">
			...
			<div align="left"
				...
				<fieldset>
					<legend class="legendtitle"><%=ChuyenNgu.get("Báo cáo xuất - nhập - tồn",nnId) %></legend>
						...
						<div style="width: 100%; float: none" align="left"
							...
							<TABLE width="70%" cellpadding="6" cellspacing="0">
								<TR>
									...
									<TD class="plainlabel">
												<input type="text" autocomplete="off" name="Sdays" class="days" value ='<%=obj.gettungay() %>'/>
									</TD>
									<TD class="plainlabel"><%=ChuyenNgu.get("Đến ngày",nnId) %></TD>
									<td >
										<input type="text" autocomplete="off" name="Edays" class="days" value ='<%=obj.getdenngay() %>'/>
									</td>
								</TR>
								<TR>
								...
								<TD class="plainlabel"><%=ChuyenNgu.get("Kênh bán hàng",nnId) %></TD>
									...
									<TD class="plainlabel"><select name="dvkdId" id="dvkdId" onchange="seach();">
											<option value="" selected>All</option>
											<% if(dvkd != null)
											   while(dvkd.next()){
                             ...
                             <TD class="plainlabel"><%=ChuyenNgu.get("Nhà phân phối",nnId) %></TD>
									...
									<TD class="plainlabel"><select name="dvdlId" id="dvdlId" onchange="seach();">
											<option value="" selected>All</option>
										<% if(dvdl != null)
											   while(dvdl.next()){
											   if(dvdl.getString("pk_seq").equals(obj.getdvdlId()))
											   { %>
											   <option value="<%= dvdl.getString("pk_seq") %>" selected><%=dvdl.getString("donvi") %></option>
											   <%}else { %>
											   <option value="<%= dvdl.getString("pk_seq") %>"><%=dvdl.getString("donvi") %></option>
											<%} }%>
									</select>
									</TD> --%>
								...
								<TR>
									<TD class="plainlabel"><%=ChuyenNgu.get("Tính theo giá",nnId) %></TD>
									<TD class="plainlabel" colspan="3">
									<select name="giatinh" id="giatinh" >
										<option value="1" selected><%=ChuyenNgu.get("Giá bán NPP",nnId) %></option>
								...
								<TR>
									<%-- <td><div id="btnSave1"><a class="button"
										href="javascript:submitform();"> <img style="top: -4px;"
											src="../images/button.png" alt=""> <%=ChuyenNgu.get("Tạo báo cáo",nnId) %>
									</a></div></td> --%>
									
									<td colspan="4"><div id="btnSave1"><a class="button"
										href="javascript:submitchitiet();"> <img style="top: -4px;"
											src="../images/button.png" alt=""> <%=ChuyenNgu.get("BC chi tiết",nnId) %>
									</a></div></td>
								</TR>
						</TABLE>
						</div>
