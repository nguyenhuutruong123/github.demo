	...
	function saveform()
	{
		...
		if(type == 2)
		{
			if(tonglg.checked)
			{
				...
				if(tongluong == "0")
				{
					alert('Ban phai nhap tong luong cua dieu kien khuyen mai lon hon 0');
					return;
				}
			}
			if(tongtn.checked)
...
<div id="main" style="width:99%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý khuyến mãi > Điều kiện khuyến mãi > Cập nhật
        </div>
    ...
    <fieldset>
    	<legend class="legendtitle"> Điệu kiện khuyến mãi</legend>
            ...
            <TABLE width="100%" cellpadding="6" cellspacing="0">
              <TR>
                    <TD width="15%" class="plainlabel" valign="top">Mã điều kiện </TD>
                ...
                <% if(dkkmBean.getHinhthuc().equals("1")){ %>
                	<TR>
	                    ...
	                    <% if(dkkmBean.getType().equals("1")){ %>
	                    	<TD class="plainlabel"><input type="number" name="tongluong" id="tongluong" value="<%= dkkmBean.getTongluong() %>" style="text-align:right" readonly></TD> 
	                    <%}else{ %>
	                    	<TD class="plainlabel"><input type="number" name="tongluong" id="tongluong" value="<%= dkkmBean.getTongluong() %>" style="text-align:right"  onkeypress="return keypress(event);"></TD> 
	                    <%} %>
                	</TR>
                <%} %>
                <% if(dkkmBean.getHinhthuc().equals("2")){ %>
                	<TR>
	                    ...
	                    <% if(dkkmBean.getType().equals("1")){ %>
	                    	<TD class="plainlabel"><input type="number" name="tongtien" id="tongtien" value="<%= dkkmBean.getTongtien() %>" style="text-align:right"  readonly> VND</TD> 
	                    <%}else{ %>
	                    	<TD class="plainlabel"><input type="number" name="tongtien" id="tongtien" value="<%= dkkmBean.getTongtien() %>" style="text-align:right"  onkeypress="return keypress(event);"> VND</TD> 
	                    <%} %>
	                </TR>   
                <%} %>
           ...
           <TABLE class="tabledetail" width="100%" border="0" cellpadding="1" cellspacing="1" >
                ...
                <% if(dkkmBean.getNhomspId().length() > 0)
                {
               			...
               			for(int i = 0; i < spSudungList.size(); i++)
               			{
              				...
              				<TR class='tbdarkrow'>
		                    	<td align="center"><input type="text" style="width:100%" value="<%= sp.getMasanpham() %>" name = "masp" onkeyup="ajax_showOptions(this,'abc',event)" style="width:100%" AUTOCOMPLETE="off"></td>
		                   		<td align="left"><input type="text" style="width:100%" value="<%= sp.getTensanpham() %>" name= "tensp" readonly></td>
		                   		<% if(dkkmBean.getType().equals("1")){ %>
		                    		<td align="center"><input type="number" name="soluong" style="width:100%" value="<%= sp.getSoluong() %>"  style="text-align:right" onkeypress="return keypress(event);"></td>
		                    	<%}else{ %>
		                    		<td align="center"><input type="number" name="soluong" style="width:100%" value="<%= sp.getSoluong() %>"  style="text-align:right" onkeypress="return keypress(event);"></td>
		                    	<%} %>
	                    	</TR>
	                ...
	                <TR class='tbdarkrow'>
	                    	<td align="center"><input type="text" style="width:100%" value="" name = "masp" onkeyup="ajax_showOptions(this,'abc',event)" style="width:100%" AUTOCOMPLETE="off"></td>
	                   		...
	                   		<% if(dkkmBean.getType().equals("1")){ %>
	                    		<td align="center"><input type="number" name="soluong" style="width:100%" style="text-align:right" onkeypress="return keypress(event);"></td>
	                    	<%} else{ %>
	                    		<td align="center"><input type="number" name="soluong" style="width:100%" style="text-align:right" onkeypress="return keypress(event);"></td>
	                    	<%} %>
	                </TR>
            ...
            <TR>
	                <TD class="plainlabel">Tổng tiền </TD>
	                    <% if(dkkmBean.getType().equals("1")){ %>
	                    	<TD class="plainlabel"><input type="number" name="tongtien" id="tongtien" value="<%= dkkmBean.getTongtien() %>" style="text-align:right" size="15" readonly> VND</TD> 
	                    <%}else{ %>
	                    	<TD class="plainlabel"><input type="number" name="tongtien" id="tongtien" value="<%= dkkmBean.getTongtien() %>" style="text-align:right" size="15" onkeypress="return keypress(event);"> VND</TD> 
	                    <%} %>
	                </TR> 
           <%} } %>             
        </div>      
