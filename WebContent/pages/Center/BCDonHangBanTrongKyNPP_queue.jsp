...
<%	
	...
	ResultSet logBaoCaoRs = obj.getLogBaoCaoRs();
	int[] quyen = new  int[6];
	if (view != null && view.length() > 0) {
		quyen = util.Getquyen("BCDonHangBanTrongKy_queue","&view="+view, userId);
	}
	else {
		quyen = util.Getquyen("BCDonHangBanTrongKy_queue","", userId);
	}
%>
...
String url = "";
if (view != null && view.length() > 0) {
	url = util.getChuyenNguUrl("BCDonHangBanTrongKy_queue","&view="+view,nnId);	
}
else {
	url = util.getChuyenNguUrl("BCDonHangBanTrongKy_queue","",nnId);	
}
 %>
...
function getBaoCao()
{
	...
	function load() {
		...
		var trangthaiId = document.getElementsByName("trangthaiId"); 
		var trangthaiText = document.getElementsByName("trangthaiText"); 
		var _link  = document.getElementsByName("_link"); 
		
		for(i=0; i < _path.length; i++)
		{
			var ttId= trangthaiId.item(i).value;
			if(ttId =='3' || ttId =='4' || ttId =='5' )
				continue;
			
			
			var _pathVal = _path.item(i).value;
			
			$.ajax({
				  type: "POST",
				  url: "../../ChatSvl?check="+_pathVal+"&action=queue",
				  data: "{''}",
				  contentType: "application/json", async: false,
			      success: function (data) {		        
			        $.each(data, function (i, item) {
			        	//console.log('oalal '+data[i].KETQUA);
		        		if(data[i].KETQUA == "SUCCESS" )
			        	{
		        			trangthaiId.item(i).value = "4";
		        			trangthaiText.item(i).innerHTML =  "Hoàn tất" ;
		        			_link.item(i).style.display = "";
			        	}
		        		else if(data[i].KETQUA == "FAIL" )
			        	{
		        			trangthaiText.item(i).innerHTML =  "Thất bại" ;
		        			trangthaiId.item(i).value = "3";
			        	}
...
<!-- END RENDER AUTO -->
	...
	<form name="frm" method="post" action="../../BCDonHangBanTrongKy_queue">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
					...
					<TR>
						<TD width="100%">
								...
								<TR>
									<TD width="98%">
										<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
											<TR class="tbheader">
												<TH width="10%"align="center">Thời điểm</TH>	
												<TH width="15%" align="center">Trạng thái</TH>		
												<TH width="15%" align="center">Link báo cáo</TH>
												
												<TH width="15%" align="center">Thông báo</TH>
							...
							<%  			
								...
								String[] ttList = {"Chưa xử lý", "Đang xử lý", "Trong hàng đợi", "Thất bại","Hoàn tất", "Hủy" };
		                               ...
		                               while (logBaoCaoRs.next()){
		                                   ...
		                                   <%} else {%>
		                                   	...
		                                   	<%}%>
		                                   		...
		                                   		<input type="hidden" name="_path" value='<%=logBaoCaoRs.getString("_path") %>'>
		                                   		<input type="hidden" name="trangthaiId" value='<%=logBaoCaoRs.getString("trangthai") %>'>
		                                   		
												<TD align="left"><div name ="thoidiem" ><%= logBaoCaoRs.getString("thoidiem")%></div></TD>                                   										
												<TD align="left"><div name ="trangthaiText" ><%= ttList[logBaoCaoRs.getInt("trangthai")]%></div></TD>
												<TD align="left"><a name ="_link" href="#" style="display: none" ><img src="../images/Display20.png" alt="Delete" title="Delete" width="20" height="20" longdesc="Delete" border=0
																			></a></TD>
												<TD align="left"><div name ="thongbao" ><%= logBaoCaoRs.getString("thongbao")%></div></TD>
														</TR>
													</TABLE>
												</TD>
