...
<form name="rpForm" method="post" action="../../Routereport">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
		...
		<TD colspan="4" align='left' valign='top' bgcolor="#FFFFFF">
			<TABLE width="100%" cellpadding="0" cellspacing="1">		
				...
				<TR>
					...
					<% } else { %>
						<input name="nppId" type="hidden" value="" />
					<% } %>

								<tr>
									<td width="19%" class="plainlabel" ><%=ChuyenNgu.get("Trạng thái NPP",nnId) %></td>
									<td class="plainlabel" >
										<table cellpadding="0" cellspacing="0">
											<tr>
												<td>
													<select name="status" onchange="submitCBO();">
														...
														<%if(obj.getStatus().equals("1")) {%>
															<option selected="selected" value="1"><%=ChuyenNgu.get("Hoạt Động",nnId) %></option>
															<option value="0"><%=ChuyenNgu.get("Không Hoạt Động",nnId) %></option>
														<%}else if(obj.getStatus().equals("0")) {%>
															<option value="1"><%=ChuyenNgu.get("Hoạt Động",nnId) %></option>
															<option selected="selected" value="0"><%=ChuyenNgu.get("Không Hoạt Động",nnId) %></option>
														<%}else { %>
														<option  value="1"><%=ChuyenNgu.get("Hoạt Động",nnId) %></option>
															<option  value="0"><%=ChuyenNgu.get("Không Hoạt Động",nnId) %></option>
														<%} %>
                              ...
                              <%}} %>                             </select>		   										</TD>
							    ...
							    <TR>
							    <TR>
										<TD width="19%" class="plainlabel" ><%=ChuyenNgu.get("Tuyến",nnId) %></TD>
								  	<TD class="plainlabel" >
										<TABLE cellpadding="0" cellspacing="0">
								<TR><TD>
								<select name="tuyenId"  >
