...
<%	
	IChiTieuNhanVienList obj=(IChiTieuNhanVienList)session.getAttribute("obj");
	...
	String url="Quản lý chỉ tiêu > Chỉ tiêu nhân viên > Sản phẩm chủ lực ";
...
<form name="tcsku" method="post" action="../../ChiTieuNhanVienSvl">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
                                ...
                                <% 
                                    ...
                                    while ( tieuchiSKU.next()){
                                        ...
                                        <%}%>
                                                ...
                                                <TD align="left"><%= tieuchiSKU.getString("Ky") %></TD>                                   
                                                ...
                                                <% if( tieuchiSKU.getString("trangthai").trim().equals("0") ) { %>
                                                	<%if(quyen[2]!=0) {%>	
					                                <A href = "../../ChiTieuNhanVienUpdateSvl?userId=<%=userId%>&update=<%= tieuchiSKU.getString("pk_seq") %>"><IMG src="../images/Edit20.png" alt="Cap nhat" title="Cap nhat" border=0></A>&nbsp;
					                               <%} %>
					                               	<%if(quyen[4]!=0){ %>
					                                <A href = "../../ChiTieuNhanVienSvl?userId=<%=userId%>&duyet=<%= tieuchiSKU.getString("pk_seq") %>"><img src="../images/Chot.png" alt="Duyet" width="20" height="20" longdesc="Duyet" border=0 onclick="if(!confirm('Bạn có muốn duyệt chỉ tiêu này?')) return false;"></A>&nbsp;
					                                <%} %>
					                                <%if(quyen[1]!=0) {%>
					                                <A href = "../../ChiTieuNhanVienSvl?userId=<%=userId%>&delete=<%= tieuchiSKU.getString("pk_seq") %>"><img src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn có muốn xóa chỉ tiêu này?')) return false;"></A>									
							                    	<%} %>
							                    <%} else { %>
							                    	<%if(quyen[3]!=0){ %>
							                    	<A href = "../../ChiTieuNhanVienUpdateSvl?userId=<%=userId%>&display=<%= tieuchiSKU.getString("pk_seq") %>"><IMG src="../images/Display20.png" alt="Hien thi" title="Hien thi" border=0></A>&nbsp;
							                    	<%} %>
							                    <%} %>
							                    </TD>
                                            </TR>
