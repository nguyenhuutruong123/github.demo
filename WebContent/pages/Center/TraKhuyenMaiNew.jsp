	...
	<script language="javascript" type="text/javascript">
		...
		function saveform()
		{
			...
			if(type == 1) //tra tien
			{
				...
				if(tongtien == "")
				{
					alert('Bạn phải nhập tổng tiền trả khuyến mãi');
					return;
				}
				if(tongtien == "0")
				{
					alert('Bạn phải nhập tổng tiền trả khuyến mãi lớn hơn 0');
					return;
				}
			}
			if(type == 2) //tra chiet khau
			{
				...
				if(chietkhau == "")
				{
					alert('Bạn phải nhập chiết khấu trả khuyến mãi');
					return;
				}
			}
...
<div id="main" style="width:99%; padding-left:2px">
    ...
    <div align="left" style="width:100%; float:none; clear:left">
    <fieldset>
    	<legend class="legendtitle"> <%=ChuyenNgu.get("Trả khuyến mãi",nnId) %></legend>
                        ...
                        <TD class="plainlabel">
                            <select name="type" id="type" onChange="submitform()">
                                <% if(trakmBean.getType().equals("1")){ %>
	                                <option value="1" selected><%=ChuyenNgu.get("Trả tiền",nnId) %></option>
	                            ...
	                            <%}else{ if(trakmBean.getType().equals("2")){ %>
	                            	<option value="2" selected><%=ChuyenNgu.get("Trả chiết khấu",nnId) %></option>
	                                <option value="1" ><%=ChuyenNgu.get("Trả tiền",nnId) %></option>
	                                <option value="3"><%=ChuyenNgu.get("Trả sản phẩm",nnId) %></option>
	                                <option value=""> </option>
	                            <%}else{ if(trakmBean.getType().equals("3")){ %>
	                            	<option value="3" selected><%=ChuyenNgu.get("Trả sản phẩm",nnId) %></option>
	                                <option value="1" ><%=ChuyenNgu.get("Trả tiền",nnId) %></option>
	                                <option value="2"><%=ChuyenNgu.get("Trả chiết khấu",nnId) %></option>
	                                <option value=""> </option>
	                            <%}else{%>
               ...
               <TABLE class="tabledetail" id="content" width="100%" border="0" cellpadding="0" style="padding-right:2px " cellspacing="1">
                    <TR class="tbheader">
                        <TH align="center" width="20%"><%=ChuyenNgu.get("Mã sản phẩm",nnId) %> </TH>
                        <TH align="left" width="70%"><%=ChuyenNgu.get("Tên sản phẩm",nnId) %></TH>
                        <% if(trakmBean.getHinhthuc().equals("1")){ %>
                       		 <TH align="center" width="10%"><%=ChuyenNgu.get("Số lượng",nnId) %></TH>
                       	<%}else{ %>
                       		<TH align="center"  style="display: none"><%=ChuyenNgu.get("Số lượng",nnId) %></TH>
                       	<%} %>
                    </TR>  
