...
function checkedAll(chk,nvcheck) {
	...
	if(nvcheck!=null && nvcheck.length>0){
		for(i=0; i<nvcheck.length; i++){
		 	if(document.nspForm.chon.checked==true){
		 		nvcheck[i].checked = true;
			}else{
				nvcheck[i].checked = false;
			}
		 }
	}
}
			...
			<TABLE width="100%" border="0" cellpadding="0"  cellspacing="1" >
				...
				<TR>
				  <TD height="100%" width="100%">
						<FIELDSET >
						<LEGEND class="legendtitle" style="color:black"><%=ChuyenNgu.get("Thông tin nhóm nhân viên",nnId) %> </LEGEND>
						<TABLE border="0" width="100%" cellpadding="6" cellspacing="0">
						   ...
						   <TR>
						  	<TD class="plainlabel">Loại Nv</TD>
						  	  	<TD class="plainlabel">
								  <SELECT name="loainvId" id="loainvId"  style="width:150px" onchange = "submitform();"  >
						  	  		<OPTION value="" ></OPTION>	
						  	  		<% if(loaiNVRs!= null) {						  	  			
						   					while (loaiNVRs.next()) {
						...
						<TABLE width="100%" border="0" cellspacing="1" cellpadding="4" >							
								<TR class="tbheader">
								<TH width="15%"><%=ChuyenNgu.get("Tên đăng nhập",nnId) %></TH>
								<TH width="55%"><%=ChuyenNgu.get("Tên nhân viên",nnId) %></TH>
								<TH width="10%"><%=ChuyenNgu.get("Điện thoại",nnId) %></TH>
								<TH width="10%"><%=ChuyenNgu.get("Chọn tất cả",nnId) %> 
									<input type="checkbox" name="chon" onClick="checkedAll(document.nspForm.spId,document.nspForm.nvselectId)"> 
									</TH>
								</TR>
