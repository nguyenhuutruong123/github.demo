...
<%@ page import="geso.dms.center.beans.tinhthunhap.ITinhthunhapList"%>
...
<%	
	ITinhthunhapList obj =(ITinhthunhapList)session.getAttribute("obj");
	ResultSet khlRs = obj.getKhlRs();
	int[] quyen = new  int[5];
	quyen = util.Getquyen("101",userId);
...
<form name="khl" method="post" action="../../TinhthunhapSvl">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
            ...
            <TR>
                <TD align="left" class="tbnavigation">
                    <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
                        <TR height="22">
                            <TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý nhân sự > Khai báo > Tính thu nhập</TD>  
        ...
        <TABLE width="100%" cellpadding="0" cellspacing="1">
            <TR>
                ...
                <TR>
                    <TD width="100%">
                        <FIELDSET>
                            <LEGEND class="legendtitle">&nbsp;Tính thu nhập &nbsp;&nbsp;	   
                                  <%if(quyen[0]!=0) {%>                  
                                ...
                                <% 
                                    ...
                                    while ( khlRs.next()){
                                        ...
                                        <%}%>
                                                ...
                                                <TD align="center"> 
                                               	 <%if(quyen[0]!=0) {%>
                                                 	<A href = "../../TinhthunhapUpdateSvl?userId=<%=userId%>&copy=<%= khlRs.getString("pk_seq") %>"><IMG src="../images/copy20.png" width="20" height="20" alt="Copy" title="Copy" border=0></A>&nbsp;
                                                	<%} %>
                                                <%if( khlRs.getString("trangthai").trim().equals("0") ) {%>
                                                	<%if(quyen[2]!=0){ %>
					                                <A href = "../../TinhthunhapUpdateSvl?userId=<%=userId%>&update=<%= khlRs.getString("pk_seq") %>"><IMG src="../images/Edit20.png" alt="Cap nhat" title="Cập nhật" border=0></A>&nbsp;
					                               	<%} %>
					                               	<%if(quyen[4]!=0){ %>
					                                <A href = "../../TinhthunhapSvl?userId=<%=userId%>&chot=<%= khlRs.getString("pk_seq") %>"><img src="../images/Chot.png" alt="Chot" title="Chốt" width="20" height="20" longdesc="Chot" border=0 onclick="if(!confirm('Bạn có muốn chốt tính thu nhập này?')) return false;"></A>&nbsp;
					                               	 <%} %>
					                               	  <%if(quyen[1]!=0){ %>
					                                <A href = "../../TinhthunhapSvl?userId=<%=userId%>&delete=<%= khlRs.getString("pk_seq") %>"><img src="../images/Delete20.png" alt="Xoa" title="Xóa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn có muốn xóa tính thu nhập này?')) return false;"></A>									
							                   			<%} %>
							                   <%} else { %>
							                   		<%if(quyen[3]!=0){ %>
							                   		<A href = "../../TinhthunhapUpdateSvl?userId=<%=userId%>&display=<%= khlRs.getString("pk_seq") %>"><IMG src="../images/Display20.png" alt="Hien thi" title="Hiển thị" border=0></A>&nbsp;
							                   		<%} %>
							                   <%} %>
							                    </TD>
                                            </TR>
