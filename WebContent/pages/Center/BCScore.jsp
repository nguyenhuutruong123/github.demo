...
<%
	...
	ResultSet nvbh = (ResultSet)obj.getRsddkd();
	ResultSet asm= (ResultSet)obj.getASM();
...
String url = util.getChuyenNguUrl("BCScoreSvl","",nnId);	
 %>
...
<!-- END RENDER AUTO -->
	...
	<form name="frm" method="post" action="../../BCScoreSvl">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
				...
				style="width: 100%; float: none; clear: left; font-size: 0.7em">
					...
					<div style="width: 100%; float: none" align="left">
						<div style="width: 100%; float: none" align="left"
							...
							<TR id="TheoThang">
									<TD class="plainlabel"><%=ChuyenNgu.get("Tháng",nnId) %></TD>
									<TD class="plainlabel">
							...
							<%if(!obj.getLoaiMenu().equals("NPP"))  {%>
                              ...
                              <%}} %>                             </select>		   										</TD>
								...
								<tr>
								  	<TD width="19%" class="plainlabel" ><%=ChuyenNgu.get("Nhân viên bán hàng",nnId) %></TD>
								  	<TD class="plainlabel" >
										<TABLE cellpadding="0" cellspacing="0">
											<TR><TD>
								<select id = "nvbh" name="nvbh"  style = "width: 250px" multiple="multiple" >
                                 <option value =""> </option>  
                               <%
                               if(nvbh != null)
                               while(nvbh.next())
                               {
                               if(obj.getDdkd().contains(nvbh.getString("pk_seq")) ) {
                              ...
                              <%}} %>                             </select>		   										</TD>
								...
								<tr>
								  	<TD width="19%" class="plainlabel" ><%=ChuyenNgu.get("Giám sát bán hàng",nnId) %></TD>
								  	<TD class="plainlabel" >
										<TABLE cellpadding="0" cellspacing="0">
											<TR><TD>
								<select id = "gsbh" name="gsbh"  style = "width: 250px" multiple="multiple" >
                                 <option value =""> </option>  
                               <%
                               if(gsbh != null)
                               while(gsbh.next())
                               {
                               if(obj.getgsbhId().contains(gsbh.getString("pk_seq")) ) {
                              ...
                              <%}} %>                             </select>		   										</TD>
									...
									<TD width="19%" class="plainlabel" ><%=ChuyenNgu.get("ASM",nnId) %></TD>
								  	<TD class="plainlabel" >
										<TABLE cellpadding="0" cellspacing="0">
											<TR><TD>
								<select id = asm name="asm"  style = "width: 250px" multiple="multiple" >
                                 <option value =""> </option>  
                               <%
                               if(asm != null)
                               while(asm.next())
                               {
                               if(obj.getASMId().contains(asm.getString("pk_seq")) ) {
