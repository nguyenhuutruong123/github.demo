...
String url = util.getChuyenNguUrl("SecondarySalesPIRTT","",nnId);	
	%>
...
<!-- END RENDER AUTO -->
	...
	<form name="frm" method="post" action="../../SecondarySalesPIRTT">  <% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%> <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
                             ...
                             <TD class="plainlabel"><%=ChuyenNgu.get("Nhà phân phối",nnId) %></TD>
								...
								<TR>
									...
									<select name="giatinh" id="giatinh">
										<option value="1" selected><%=ChuyenNgu.get("Giá mua NPP",nnId) %></option>
										<option value="2"><%=ChuyenNgu.get("Giá bán NPP",nnId) %></option>
										<!-- <option value="3">Giá tồn</option> -->
									</select>
									</TD>
								</TR>
								<%-- <TR>
								<TD class="plainlabel">Hàng đang đi đường</TD>
									<TD class="plainlabel">
									<%
										if(obj.getdiscount().equals("1")){
											%>
											<input type="radio" name="instransit" value="1" checked="checked" /> Không &nbsp; &nbsp;
											<input type="radio" name="instransit" value="0" /> Có 
											<%
										}
										else
										{
											%>
											<input type="radio" name="instransit" value="1"  /> Có &nbsp; &nbsp;
										<input type="radio" name="instransit" value="0" checked="checked" /> Không 
											<%
										}
									%>
